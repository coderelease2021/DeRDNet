...
from baseline.unit import MyData, DataLoader
...
def train(model, optimizer, train_dataloder, val_dataloader, test_dataloader, config, record):
    min_loss = 1000
    t_count = 0
    for epoch in range(config.train_epoch):
        ...
        _, _, _, test_loss, _ = test(model, optimizer, val_dataloader, config, record, False)
        if test_loss < min_loss:
            print('New Test Loss:%f' % test_loss)
            t_count = 1
            min_loss = test_loss
            _, _, _, test_loss, result_one = test(model, optimizer, test_dataloader, config, record, True)
...
def test(model, optimizer, test_dataloder, config, record, is_test):
    if is_test:
        print('Testing')
    else:
        print('Validation')
...
def start():
    ...
    model, optimizer, train_dataloader, val_dataloader, test_dataloader = build(config)
    train(model, optimizer, train_dataloader, val_dataloader, test_dataloader, config, record)
def start_test():
    try:
        torch.cuda.set_device(config.gpuid)
    except:
    ...
    for k in range(5):
        record = open('record.txt', 'wb', buffering=0)
        model, optimizer, train_dataloader, val_dataloader, test_dataloader = build(config)
        best_result_one = train(model, optimizer, train_dataloader, val_dataloader, test_dataloader, config, record)
