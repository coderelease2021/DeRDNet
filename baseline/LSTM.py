...
from baseline.LSTM import BaseModel
class Config(object):
...
class TextEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.LSTM = torch.nn.GRU(256, 256, batch_first=True, num_layers=1)
        self.embedding_layer = torch.nn.Embedding(30522, 256)

    def forward(self, x):
        text = x[0]
        text = self.embedding_layer(text)
        mask = x[1]
        outputs = self.LSTM(text)
        return outputs[1][-1]
